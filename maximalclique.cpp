 ...
 *
 * C++ sequential implementation of the max clique algorithm
 * described in:
 *
...
namespace
{
    ...
    class BitSet
    {
        ...
        public:
            ...
            auto equals(std::vector <unsigned>c) -> bool
            {
                int l = popcount();
    ...
    struct MaximalClique
    {
        ...
        MaximalClique(const GraphFromFile & g) :
            order(g.first)
        {
     ...
     */
    auto max_clique(const GraphFromFile & graph) -> MaximalCliqueResult
    {
        return select_graph_size<MaximalClique, MaximalCliqueResult>(AllGraphSizes(), graph);
    }
}
